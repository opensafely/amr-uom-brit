...
df$indication <- ifelse(is.na(infection),"uncoded","coded")
...
bkg_colour <- "white"
figure_indication_strata <- ggplot(df.model, aes(x = as.Date("2019-01-01"), y = value, group = factor(age_group), col = factor(age_group), fill = factor(age_group))) +
