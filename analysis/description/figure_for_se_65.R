...
p_1 <- ggplot(df.model, aes(y=value, x=monPlot)) + 
  ...
  labs(
    title = "Broad-spectrum percentage_65+_1",
...
ggsave(
  plot= p_1,
  filename="figure_broad_65_1.jpeg", path=here::here("output"),
)  

df.model$value <- df.model$numOutcome/df.model$numEligible

write_csv(df.model, here::here("output", "figure_broad_65_1_table.csv"))
...
p_1 <- ggplot(df.model, aes(y=value, x=monPlot)) + 
  ...
  annotate(geom = "rect", xmin = as.Date("2020-03-01"),xmax = as.Date("2020-06-01"),ymin = -Inf, ymax = Inf,fill="grey80", alpha=0.5)+
  labs(
    title = "Broad-spectrum percentage_65+_2",
    caption = "National lockdown time in grey background. ",
    y = "Percentage",
    x=""
  )+
...
ggsave(
  plot= p_1,
  filename="figure_broad_65_2.jpeg", path=here::here("output"),
)  

df.model$value <- df.model$numOutcome/df.model$numEligible

write_csv(df.model, here::here("output", "figure_broad_65_2_table.csv"))
rm(df.broad_total,df.all,df.model,df.broad)
...
p_1 <- ggplot(df.model, aes(y=value, x=monPlot)) + 
  ...
  annotate(geom = "rect", xmin = as.Date("2020-03-01"),xmax = as.Date("2020-06-01"),ymin = -Inf, ymax = Inf,fill="grey80", alpha=0.5)+
  labs(
    title = "Broad-spectrum percentage_65+_3",
    caption = "National lockdown time in grey background. ",
    y = "Percentage",
    x=""
  )+
...
ggsave(
  plot= p_1,
  filename="figure_broad_65_3.jpeg", path=here::here("output"),
)  

df.model$value <- df.model$numOutcome/df.model$numEligible

write_csv(df.model, here::here("output", "figure_broad_65_3_table.csv"))
rm(df.broad_total,df.all,df.model,df.broad)