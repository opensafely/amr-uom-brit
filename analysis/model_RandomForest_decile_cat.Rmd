...
rf.model=readRDS(here::here("output","model_RF_weighted.rds"))
print(rf.model)
...
# select columns
```{r }
col=c("patient_id","patient_index_date","case","subclass","case","total_ab_group","ab_types_group",
      "exposure_period_group","recent_ab_days_group",
      "broad_ab_prescriptions_group",
      "interval_med_group" ,"interval_sd_group",
       "ethnicity_6","bmi","bmi_cat","CCI","Charlson","smoking_cat_3","imd","care_home","covrx_ever","flu_vaccine","care_home_type","hospital_counts","total_ab","ab_types","exposure_period","recent_ab_days","broad_ab_prescriptions","interval_med" ,"interval_sd")
...
prob = predict(rf.model, data=train)$prediction[,2]
summary(prob)
hist(prob)
...
prob = predict(rf.model, data=valid)$prediction[,2]
