...
outlier=quantile(df$sum,0.99)
DF=df%>% filter(sum<=outlier)
counts=table(DF$sum)
outlier
length(counts)

outlier=quantile(df$sum,0.995)
DF=df%>% filter(sum<=outlier)
counts=table(DF$sum)
outlier
length(counts)

outlier=quantile(df$sum,0.999)
DF=df%>% filter(sum<=outlier)
counts=table(DF$sum)
outlier
length(counts)
...
df01=df%>%filter(Rx_Nitrofurantoin>0)
ggplot(df01, aes(y=Rx_Nitrofurantoin,x=case)) + 
    geom_boxplot() 

df02=df%>%filter(Rx_Trimethoprim>0)
ggplot(df02, aes(y=Rx_Trimethoprim,x=case)) + 
    geom_boxplot() 

df03=df%>%filter(Rx_Amoxicillin>0)
ggplot(df03, aes(y=Rx_Amoxicillin,x=case)) + 
    geom_boxplot() 

df04=df%>%filter( Rx_Flucloxacillin>0)
ggplot(df04, aes(y= Rx_Flucloxacillin, x=case)) + 
    geom_boxplot() 
df05=df%>%filter(Rx_Doxycycline>0)
ggplot(df05, aes(y=Rx_Doxycycline,x=case)) + 
    geom_boxplot() 
...
```{r eval=FALSE, include=FALSE}
data.frame(prop.table(table(df$diff_N)))
data.frame(prop.table(table(df$diff_T)))
data.frame(prop.table(table(df$diff_A)))
data.frame(prop.table(table(df$diff_F)))
data.frame(prop.table(table(df$diff_D)))
