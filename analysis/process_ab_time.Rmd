...
DF$last_date=do.call(pmax, c(DF[ab_date], na.rm = TRUE))
...
DF$AB_date_1=ifelse(is.na(DF$AB_date_1),NA,DF$AB_date_1)

DF$last_date=ymd(DF$last_date)
DF$AB_date_1=ymd(DF$AB_date_1)

#DF$exposure_period=DF$last_date-DF$AB_date_1
DF$exposure_period=difftime(DF$last_date,DF$AB_date_1, units = "day")
...
```{r}
DF$recent_ab_days=difftime(DF$patient_index_date,DF$last_date, units = "day")
```
...
for (i in 1:60) {
  DF[,ab_date[i]]=ifelse(is.na(DF[,ab_date[i]]), NA,  DF[,ab_date[i]])
   DF[,ab_date[i]]=ymd( DF[,ab_date[i]])
}
...
DF[,137+i]=difftime(DF[,ab_date[i+1]], DF[,ab_date[i]], units = "day" )
...
df=df%>% select("patient_id","patient_index_date","total_ab","ab_prescriptions","broad_ab_prescriptions","ab_types","case","subclass")
