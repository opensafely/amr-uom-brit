...
main <- function(condition) {
  df_cases <- load_dataset(paste0("output/matched_cases_", condition, "_amr.csv"))
  df_matches <- load_dataset(paste0("output/matched_matches_", condition, "_amr.csv"))
  ...
  df_cases_ab <- load_prepared_df(paste0("output/processed/input_case_", condition, "_abvar_amr.rds"), df_ab_vars)
  df_cases_oth <- load_prepared_df(paste0("output/processed/input_case_", condition, "_othvar_amr.rds"), df_oth_vars)

  df_matches_ab <- load_prepared_df(paste0("output/processed/input_control_", condition, "_abvar_amr.rds"), df_ab_vars)
  df_matches_oth <- load_prepared_df(paste0("output/processed/input_control_", condition, "_othvar_amr.rds"), df_oth_vars)
  ...
  saveRDS(merged_data, here("output", "processed", paste0("model_", condition, "_amr.rds")))
}
