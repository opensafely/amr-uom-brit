...
df=df%>% select("patient_id","patient_index_date", "case","total_ab","ab_types","exposure_period" ,"recent_ab_days", "broad_prop" ,"interval_mean" , "interval_CV",abtype, "ab_6w_binary" , "AB_6wk","set")
...
```{r}
model=clogit(case ~recent_ab_days + CCI +covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass),data) 
...
```{r}
model=clogit(case ~interval_mean + CCI + covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass),data) 
