...
plota$group <- factor(plota$group, levels = label1)


p <- ggplot(data=plota, aes(y=type, x=OR, xmin=CI_L, xmax=CI_U,col=group,fill=group)) +
  geom_point() + 
...
```{r echo=TRUE}
p <- ggplot(data=plotb, aes(y=type, x=OR, xmin=CI_L, xmax=CI_U,col=group,fill=group)) +
  geom_point() + 
  geom_errorbarh(height=.1) +
  geom_vline(xintercept=1, color='black', linetype='dashed', alpha=.5) +
  theme_minimal()
