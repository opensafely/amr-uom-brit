...
# import dataset
```{r }
df <- read_rds(here::here("output","matched_ab_2.rds"))
table(df$case)
nrow(df)
...
DF<- read_rds(here::here("output","abtype79_2.rds"))
table(DF$case)
names(DF)
...
saveRDS(col,here::here("output","abtype_2.rds"))
...
```{r}
saveRDS(train_X, here::here("output","train_2.rds"))
saveRDS(valid_X, here::here("output","valid_2.rds"))
...
df1=df%>% filter(set==1)
df2=df%>% filter(set==2)
saveRDS(df1,here::here("output","train_cat_2.rds"))
saveRDS(df2,here::here("output","valid_cat_2.rds"))