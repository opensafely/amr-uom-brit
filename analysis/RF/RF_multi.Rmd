...
train_X=train_X[col]
saveRDS(train_X,here::here("output","development_1control.rds"))
```

# RF model
```{r}
library(randomForest)
#train_X=train%>% sample_n(10000)
sample_n=as.numeric(table(train$case)[2]*0.5)
sample_n
...
train_X=train[col]
set.seed(123)
rf.model<- randomForest(formula = case ~ total_ab_group+ab_types_group+exposure_period_group+recent_ab_days_group+broad_ab_prescriptions_group+interval_mean_group+interval_sd_group, data = train_X, 
                           replace = TRUE,
                         #sampsize=c(sample_n*6,sample_n*1), 
                           sampsize=0.5,
                          strata=train_X$case,
                       #   samplsize=0.4,
                           ntree =  1000,
