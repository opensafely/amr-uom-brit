...
```{r eval=FALSE, include=FALSE}
for (i in c(1,2,5)) {# just replace total ab
...
rf.model= ranger(formula = case ~ total_ab+ab_types+exposure_period+recent_ab_days+broad_ab_prescriptions+interval_mean+interval_sd, 
               ...
               #    min.node.size = 300, # node to split
                   max.depth= 50,
                   min.bucket=300,#terminal node
                   mtry = 3,
                   sample.fraction = 0.7, # fraction of observations to sample
                   importance = "impurity",
                   replace = TRUE,
                   probability = TRUE
                          )
 ...
 # add OOB error to grid
  #  hyper_grid$OOB_RMSE[i] <- sqrt(rf.model$prediction.error)
#
#hyper_grid
...
data$prob <- predictions(rf.model)[,2]# votes of being case
