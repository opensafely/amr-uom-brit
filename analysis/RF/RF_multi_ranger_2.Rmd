...
# random select subclass (1:2 matched)
```{r}
nrow(train)
...
train.0_1=train.0%>% filter(n<2)
train.0_2=train.0%>% filter(n>1) %>% group_by(subclass) %>%sample_n(2)
...
valid.0_1=valid.0%>% filter(n<2)
valid.0_2=valid.0%>% filter(n>1) %>% group_by(subclass) %>%sample_n(2)
...
saveRDS(rf.model,here::here("output","RF_model_ranger_2.rds"))
```
...
```{r}
saveRDS(development,here::here("output","development_ranger_2.rds"))
saveRDS(validation,here::here("output","validation_ranger_2.rds"))
