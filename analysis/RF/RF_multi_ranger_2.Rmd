...
train.0=train.0 %>% group_by(subclass) %>% mutate(n=nrow())
train.0_1=train.0%>% filter(n<2)
train.0_2=train.0%>% filter(n>1) %>%sample_n(2)
...
valid.0=valid.0 %>% group_by(subclass) %>% mutate(n=nrow())
valid.0_1=valid.0%>% filter(n<2)
valid.0_2=valid.0%>% filter(n>1) %>%sample_n(2)
