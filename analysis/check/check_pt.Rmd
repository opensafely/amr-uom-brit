...
# covid infection without severe outcome
```{r}
...
N=nrow(df)
round(N/5)*5
R5=c(round(N/5)*5,"infection_excluding_severe_outcome")
...
N=nrow(df0)
round(N/5)*5
R8=c(round(N/5)*5,"matched.contr")

df1=df%>%filter(case==1)
N=nrow(df1)
round(N/5)*5
R9=c(round(N/5)*5,"matched.case")

```

```{r}
tbl=rbind(R1,R2,R3,R4,R5,R6,R7,R8,R9)
