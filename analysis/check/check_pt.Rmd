...
# primary care
```{r}
df1 <- read_csv(here::here("output","input_covid_primarycare.csv"))
N=nrow(df1)
round(N/5)*5
R2=c(round(N/5)*5,"GP")
...
# SGSS
```{r}
df2 <- read_csv(here::here("output","input_covid_SGSS.csv"))
N=nrow(df2)
round(N/5)*5
...
N=nrow(df.1)
round(N/5)*5
R8=c(round(N/5)*5,"incident_case")
```
...
N=nrow(df)
round(N/5)*5
R9=c(round(N/5)*5,"merged_case_control")
...
N=nrow(df.0)
round(N/5)*5
R10=c(round(N/5)*5,"control_ab>1")


df.1=df.1%>% filter(had_antibiotic>1)# filter ab>1
N=nrow(df.1)
round(N/5)*5
R11=c(round(N/5)*5,"case__ab>1")
```
...
N=nrow(df0)
round(N/5)*5
R12=c(round(N/5)*5,"matched.contr")

df1=df%>%filter(case==1)
N=nrow(df1)
round(N/5)*5
R13=c(round(N/5)*5,"matched.case")

```

```{r}
tbl=rbind(R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,R12,R13)
