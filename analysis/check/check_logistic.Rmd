...
logistic model
```{r }
rf.model <- glm( case ~total_ab+ab_types+exposure_period+recent_ab_days+broad_ab_prescriptions+interval_med+interval_sd, data = train, family = binomial)
print(rf.model)
```
...
nrow(train)
saveRDS(train, here::here("output","development_logi.rds"))
...
prob <- predict(rf.model,valid, type = 'response')
summary(valid$prob)
hist(valid$prob)
...
saveRDS(valid, here::here("output","validation_logi.rds"))
```
