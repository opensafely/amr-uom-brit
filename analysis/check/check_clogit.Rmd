...
train$case=as.numeric(train$case) #1/0
train$decile=as.factor(train$decile)
...
```{r}
prob <- predict(model, newdata=data.frame(train), type = 'risk')
```
...
valid$case=as.numeric(valid$case) #1/0
valid$decile=as.factor(valid$decile)
#col <- c("case","decile","subclass","CCI","covrx_ever","bmi_cat","care_home","flu_vaccine","smoking_cat_3","imd","ethnicity_6")
#train=train%>% mutate_at(col, as.factor)
