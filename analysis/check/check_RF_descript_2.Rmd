...
```{r}
model_rf=readRDS(here::here("output","RF_model_2.rds"))
...
# read table
```{r}
data=read_rds(here::here("output","all_ranger_2.rds"))
...
write.csv(table,here::here("output","crosstab_2.csv"))
...
write.csv(table,here::here("output","factor.range_2.csv"))
...
for (i in 1:length(explanatory)) {
  ...
  temp=vector()
  for (j in 1:nrow(DF)) {
  temp[j]= paste0(j," (",round(DF[j,1]),",",round(DF[j,2]),")")
  }
