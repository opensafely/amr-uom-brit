...
input_data <- data %>%
  dplyr::select(patient_id, EVENT, TEVENT, age, sex, region, imd, ethnicity, bmi, smoking_status_comb, charlsonGrp, ab_3yr, ab_30d, has_uti, has_urti, has_lrti, has_sinusitis, has_ot_externa, has_otmedia, covid) %>%
  group_by(patient_id) %>%
  sample_n(1) %>%
  ungroup()
