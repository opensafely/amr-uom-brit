################################################################################
# Part 1: sinusitis model development process                                        #
################################################################################
...
data <- input_data %>% filter(has_sinusitis)
...
ggsave(plot_part6, dpi = 700,
       filename = "spline_age_sinusitis_all.jpeg", path = here::here("output"))
...
model_selected <- coxph(Surv(TEVENT, EVENT) ~ sex + age3_spline + region + imd + ethnicity + bmi + smoking_status_comb + charlsonGrp + ab_30d + ab_3yr + covid, data = training)
...
jpeg(here::here("output", "sinusitis_KM_Curves_all.jpeg"))
...
write_csv(pm, here::here("output", "sinusitis_model_evaluation_all.csv"))
...
jpeg(here::here("output", "sinusitis_High_Low_Risk_Patients_all.jpeg"))
...
jpeg(here::here("output", "sinusitis_survival_plot_baseline_survival_curves_all.jpeg"))
...
jpeg(here::here("output", "sinusitis_survival_plot_baseline_survival_curves2_all.jpeg"))
...
write_csv(results, here::here("output", "sinusitis_model_HR_all.csv"))
...
write_csv(results, here::here("output", "sinusitis_sex_age_band_HR_all.csv"))
...
write_csv(results, here::here("output", "sinusitis_sex_age_spline_HR_all.csv"))
...
write_csv(all_results, here::here("output", "sinusitis_sex_age_var_HR_all.csv"))
...
write_csv(results, here::here("output", "sinusitis_sex_age_band_full_HR_all.csv"))
...
write_csv(pm_ext, here::here("output", "sinusitis_model_external_all.csv"))
...
jpeg(here::here("output", "sinusitis_30day_calibration_all.jpeg"))
...
jpeg(here::here("output", "sinusitis_30day_re-calibration.jpeg"))
