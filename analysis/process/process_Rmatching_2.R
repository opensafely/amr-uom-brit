...
write_rds(DF_type, here::here("output", "abtype79_2.rds"))
...
write_rds(df[comor], here::here("output", "comor17_2.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_2.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_2.rds"))
