...
listFiles <- list.files(pattern="input_ab_time_general_2020+.*csv")
listFiles
...
col=c("patient_index_date","patient_id","subclass","case",
      "wave","sex","age","age_cat","stp","region",
      "total_ab","ab_prescriptions","broad_ab_prescriptions","ab_types",
...
data$broad_prop_total=data$broad_ab_prescriptions/data$total_ab
...
# prescribe times
data$prescribe_time_0=ifelse(data$prescribe_times==0,0,1) #1= ab user ,0=non 
data$prescribe_time_1=ifelse(data$prescribe_times==1,1,0) 
data$prescribe_time_2=ifelse(data$prescribe_times==2,1,0) 
data$prescribe_time_3=ifelse(data$prescribe_times>=3,1,0) # missing SD, CV
```

```{r}
write_rds(data, here::here("output", "matched_ab_general_2021.rds"))
```
