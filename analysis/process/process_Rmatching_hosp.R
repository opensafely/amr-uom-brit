...
DF1=read_csv("input_outcome_hosp.csv")

## add variables to extracted cohort:"subclass","case", 
DF2 <- read_rds("matched_patients_hosp.rds")
...
write_rds(DF_type, here::here("output", "abtype79_hosp.rds"))
...
write_rds(df[comor], here::here("output", "comor17_hosp.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_hosp.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_hosp.rds"))
