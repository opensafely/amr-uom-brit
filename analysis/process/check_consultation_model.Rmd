...
Interrupted time-series analyses are conducted to compare the difference of consultation rate before and during pandemic. Pre-pandemic period for Covid-19 is defined as 1st January 2019 to 31st December 2019, and the post-pandemic period for Covid-19 is defined as 1st April 2020 to 31st December 2021. Assessment period is defined as every month to acquire the counts of infection consultations and the number of populations. Negative binomial regression model is used to adjust for regions, age categories, month(seasonality), and indecent_label for 6 types of infections. 
...
df19 = readRDS(here::here("output","measures","basic_record_2019.rds"))
df19=df19%>% dplyr::select(practice,region)

df20 = readRDS(here::here("output","measures","basic_record_2020.rds"))
df20=df20%>% dplyr::select(practice,region)

df21 = readRDS(here::here("output","measures","basic_record_2021.rds"))
...
df=df%>%select(covid, region, age_cat, month, hx_pt, infection_counts,population)
...
This plot showed the distribution of infection consultation counts among independent variables.
...
#### prevalent:same infection in recent 90 days before index
...
### compare models without covid time
Since covid time is considered as the most important variables of interest, so can compare two models with and without covid time.
