...
DF1=read_csv("input_outcome_ratio1.csv")

## add variables to extracted cohort:"subclass","case", 
DF2 <- read_rds("matched_patients_ratio1.rds")
...
write_rds(DF_type, here::here("output", "abtype79.rds_ratio1"))
...
write_rds(df[comor], here::here("output", "comor17_ratio1.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_ratio1.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_ratio1.rds"))
