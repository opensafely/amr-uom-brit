...
df%>%
  summary_factorlist("case", "CCI", fit_id = TRUE) %>% 
  ff_merge(
    survival::clogit(case ~  CCI + strata(subclass),df) %>% 
...
df%>%
  summary_factorlist("case", "covrx_ever", fit_id = TRUE) %>% 
  ff_merge(
    survival::clogit(case ~ covrx_ever + strata(subclass),df) %>% 
      fit2df(estimate_name = "OR (95% CI; case-control)"),
    last_merge = TRUE
  )%>% select(-c("unit","value"))
