...
case_ab <- select(case_ab,patient_id,patient_index_date,ab_type_num)
control_ab <- select(control_ab,patient_id,patient_index_date,ab_type_num)
...
names(DF)[1]="type"
write_csv(DF, here::here("output", "model_ab_type_full.csv"))

ab_type_num <- DF[49:51,]

mod=clogit(case ~ ab_type_num + strata(set_id), df)
sum.mod=summary(mod)
sum.mod
result=data.frame(sum.mod$conf.int)
DF=result[,-2]
...
setDT(DF, keep.rownames = TRUE)[]
names(DF)[1]="type"
write_csv(DF, here::here("output", "model_ab_type_unadjust.csv"))