...
#setwd("/Users/user/Documents/GitHub/amr-uom-brit/output")

rm(list=ls())
## add variables to extracted cohort:"set_id","case", "match_counts"   

# import data
#df1_list=c("matched_combined_general_population_infection.csv","matched_combined_infection_hosp.csv","matched_combined_hosp_icu_death.csv")
#df2_list=c("input_outcome_1.csv","input_outcome_2.csv","input_outcome_3.csv")


df1 <- read_csv("matched_combined_general_population_infection.csv",
                col_types = cols_only(
                  patient_id ='d',
                  age = 'd',
                  sex= 'c',
...
df2=read_csv("input_outcome_1.csv")

df=merge(df2,df1,by=c("patient_id","age","sex","stp"),all.x=T)
...
df$died_ever=iflese(df$ons_died_date < df$patient_index_date, 1, 2)
df$died_ever=ifelse(is.na(df$ons_died_date),0,df$died_ever)


#1 deregister before patient_index_date/#2 deregister after patient_index_date/# 0 never deregister
df$dereg_ever=iflese(df$dereg_date < df$patient_index_date, 1, 2)
