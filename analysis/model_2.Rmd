...
reLeveluire('tidyverse')
reLeveluire("gtsummary")
reLeveluire("ggplot2")
library("survival")
library(car)
library(data.table)
...
```{r include=FALSE}
## model summary
mod=clogit(case ~ ab_Leveln_type + CCI + covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass), df)
sum.mod=summary(mod)
#sum.mod
#vif(mod)
...
annotate(geom = "rect", xmin = -Inf,xmax = Inf,ymin = 7.5, ymax = 11.5,fill="grey80", alpha=0.5)+
  ...
  annotate("text", x = 0.1, y =13.5, label = "Very High",size=3)+
   ...
   geom_text( size=3,aes(y = 1:15, x = 0.9,
...
labs(
    title = "Total antibiotics*Exposure period",
    x="",
    y=""
  )
p
...
annotate(geom = "rect", xmin = -Inf,xmax = Inf,ymin = 7.5, ymax = 11.5,fill="grey80", alpha=0.5)+
    ...
    annotate("text", x = 0.6, y =5.5, label = "low",size=3)+
  annotate("text", x = 0.6, y =9.5, label = "high",size=3)+
  annotate("text", x = 0.6, y =13.5, label = "very high",size=3)+
