...
df191=read_csv("input_control_ab_6w_191.csv")
df192=read_csv("input_control_ab_6w_192.csv")
df201=read_csv("input_control_ab_6w_201.csv")
df202=read_csv("input_control_ab_6w_202.csv")
df211=read_csv("input_control_ab_6w_211.csv")
df212=read_csv("input_control_ab_6w_212.csv")
df221=read_csv("input_control_ab_6w_221.csv")
...
df$ab_type_num_6w = case_when(
  df$ab_types_6w == 0 ~ "0",
  df$ab_types_6w == 1 ~ "1",
  df$ab_types_6w >1 & df$ab_types_6w <4 ~ "2-3",
  df$ab_types_6w > 3 ~ ">3",)

output <- select(df,patient_id,patient_index_date,ab_frequency_6w,ab_type_num_6w)

# Save output ---
output_dir <- here("output", "processed")
fs::dir_create(output_dir)
saveRDS(object = output,
        file = paste0(output_dir, "/input_", "control_ab_6w", ".rds"),
        compress = TRUE)