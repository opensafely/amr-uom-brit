...
write_rds(df[col], here::here("output", "abtype79_3.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_3.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_3.rds"))
