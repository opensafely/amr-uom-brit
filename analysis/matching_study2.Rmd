...
df1=read_csv(here::here("output","case_covid_icu_death.csv"))
df1$case=1
df2=read_csv(here::here("output","control_covid_hosp.csv"))
df2$case=0
...
write_rds(m.data1, here::here("output", "matched_patients_study2.rds"))

m.data2 <- match.data(m.out1) # unique patient ID
m.data2=m.data2%>%select("patient_id","patient_index_date") 
write.csv(m.data2,here::here("output","matched_patients_id_study2.csv"))

unmatched=subset(df, !df$patient_id %in% m.data1$patient_id)
write_rds(unmatched,here::here("output","unmatched_patients_study2.rds"))
