...
df1 <- read_rds(here::here("output","train.rds"))
df2 <- read_rds(here::here("output","valid.rds"))
df=rbind(df1,df2)
rm(df1,df2)
table(df$case)
str(df)
...
df$broad_ab_prescriptions=relevel(as.factor(df$broad_ab_prescriptions),ref= "0")
...
df$pred_y <- predict(rf.model, train, type = 'response')
summary(df[,"pred_y"])
