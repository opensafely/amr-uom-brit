...
# import data
df <- read_csv(here::here("output", "matched_outcome1.csv"))
```
...
#### Cases: COVID hospital admission
##### **count:`r num.case`**

*inclusion*

1. patients admitted to hospital with primary diagnosis as COVID

2. select **incident** case_index_date

*exclusion*

1. COVID hospital admission records before case_index_date

2. COVID death records before case_index_date
...
#### Controls: COVID infection
##### **count:`r num.contr`**

*inclusion*

1. primary care records with COVID diagnosis or The Second Generation Surveillance System (SGSS) with positive results

2. select **incident** case_index_date

*exclusion*

1. COVID infection cords before case_index_date

2. COVID hospital admission records before case_index_date

3. COVID death records before case_index_date
...
The plots below show the distribution about means of age within each "set_id" that indicated case and control pairs.
```{r  echo=FALSE }
...
df.m=df%>% select(case, age,age_cat, sex, region)
...
```{r  echo=FALSE }
df.con <- df %>% select(case, wave, ethnicity, bmi, bmi_cat_6,care_home,imd, smoking_cat_3,covrx,flu_vaccine,Charlson,CCI)
```
...
```{r  echo=FALSE }
df.ab <- df %>% select(case,ab_types,ab_prescriptions,ab_qn,broad_ab_prescriptions,br_ab_qn,lastABtime,interval,ab_freq,ab_freq.type)

#df.ab$ab_freq=as.numeric(df.ab$ab_freq)
