...
# Define the types and generate the input file names
...
process_data <- function(input_files, output_file) {
  ...
  # Generate full path to the input files
  input_files_path <- here::here("output", input_files)
  
  # Check if all input files exist
  if (all(file.exists(input_files_path))) {
    # Read all files
    df_list <- lapply(input_files_path, function(input_file) {
      # Data processing...
        # Data processing
...
# Apply function to each type
for (type in types) {
  input_files_type <- grep(type, input_files, value = TRUE)
  output_file <- paste0("input_control_", type, "_var_2.rds")
