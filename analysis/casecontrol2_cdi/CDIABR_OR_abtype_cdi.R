...
main <- function(condition) {
  # Read the dataset
  df <- readRDS(here::here("output", "processed", paste0("model_", condition, "_cdi.rds")))
...
write_csv(as.data.frame(freq_table), here::here("output", paste0(condition, "_ab_history_table_cdi.csv")))
...
df$exposure_ab = relevel(as.factor(df$exposure_ab), ref="count_0_type_0")
  ...
  write_csv(combined_df, here::here("output", paste0(condition, "_model_result_abtype_cdi.csv")))
}
