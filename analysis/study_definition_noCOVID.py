...
study = StudyDefinition(
    ...
    population=patients.satisfying(
        ...
        AND NOT stp = ""
        AND NOT has_outcome_primarycare
        AND NOT has_outcome_SGSS
        AND NOT has_outcome_hosp
        AND NOT has_outcome_CPNS
        AND NOT has_outcome_ONS
        """,
        ...
        has_outcome_primarycare=patients.with_these_clinical_events(
            any_primary_care_code,        
            on_or_after="index_date",
        ),
        has_outcome_SGSS=patients.with_test_result_in_sgss(
            pathogen="SARS-CoV-2",
            test_result="positive",
            on_or_after="index_date",
        ),
        has_outcome_hosp=patients.admitted_to_hospital(
            with_these_primary_diagnoses=covid_codelist,  
            on_or_after="index_date",
        ),
        has_outcome_CPNS= patients.with_death_recorded_in_cpns(
            on_or_after="index_date",
        ),
        has_outcome_ONS=patients.with_these_codes_on_death_certificate(
            covid_codelist,
            on_or_after="index_date",
        ),
