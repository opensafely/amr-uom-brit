...
```{r}
train.sub=train%>%group_by(case, subclass)%>%sample_n(1)
valid.sub=valid%>%group_by(case, subclass)%>%sample_n(1)

table(train.sub$case)
table(valid.sub$case)

train.sub=train.sub%>%ungroup(case,subclass)
valid.sub=valid.sub%>%ungroup(case,subclass)

```

import saved model
```{r }
rf.model=readRDS(here::here("output","model_RF_random_1_control.rds"))
print(rf.model)
```
