...
df1=read_csv(here::here("output","case_covid_icu.csv"))
df1$case=1
...
write_rds(m.data1, here::here("output", "matched_patients_icu.rds"))

m.data2 <- match.data(m.out1) # unique patient ID
m.data2=m.data2%>%select("patient_id","patient_index_date") 
write.csv(m.data2,here::here("output","matched_patients_id_icu.csv"))

unmatched=subset(df, !df$patient_id %in% m.data1$patient_id)
write_rds(unmatched,here::here("output","unmatched_patients_icu.rds"))
