...
```{r}
corrplot(cor(df[,numeric]), method = "number")
```

```{r}
for (i in 1:length(factor)) {
print(factor[i])  
table(df$case, df[,factor[1]])
prop.table(table(df$case, df[,factor[i]]))
}
