...
# input variable
```{r }
col=c( "case","total_ab","ab_types","prescribe_times","exposure_period","recent_ab_days", "broad_prop" ,"broad_ab_prescriptions","interval_mean","interval_sd","interval_CV","length_mean", "length_sd","length_CV","prescribe_time_0","prescribe_time_1","prescribe_time_2","prescribe_time_3","interval_med" ,"length_med")
    #  "AB_1_type","AB_6wk_type" , ,"ab_6w_binary" , "AB_6wk")

numeric=c("total_ab","ab_types","prescribe_times",
...
for (i in 1:length(numeric)){
    ...
    for (j in 1:10) {
    decile_value[j]=quantile(data,(j/10),na.rm=T)
    }
    
    groups=decile_value
    groups=ifelse(data <=decile_value[1],1,groups)
    
    for (k in 1:9) {
          groups=ifelse(groups >decile_value[k] & groups <=decile_value[k+1],k+1,groups) 

    }

    
    patient_number=table(groups)
    patient_percent=prop.table(table(groups))*100
    print(rbind(decile_value, patient_number,patient_percent))
...
print(factor[i])  
print(table(df$case, df[,factor[i]]))
print(prop.table(table(df$case, df[,factor[i]]))*100)
}
