...
all_files <- list.files(here::here("output"), pattern = "week_")
outcomes <- stringr::str_remove_all(all_files, c("week_|.csv"))
...
for(ii in 1:length(outcomes)){
  load_file <- read.csv(here::here("output", paste0("week_", outcomes[ii], ".csv")))
...
its_function <- function(outcomes_vec = outcomes,
  ...
  write_csv(main_plot_data, here::here("output", "week_overall_predicted_table.csv"))
  ...
  ggsave(
    plot= plot1,
    filename="week_overall_predicted_plot.jpeg", path=here::here("output"),
  )    
    ...
    write_csv(interaction_tbl_data, here::here("output", "week_overall_forest_C_table.csv"))
  ...
  ggsave(
    plot= fp2,
    filename="week_overall_forest_C.jpeg", path=here::here("output"),
  )  
  ...
  write_csv(forest_plot_df, here::here("output", "week_overall_forest_B_table.csv"))
  ...
  ggsave(
    plot= fp,
    filename="week_overall_forest_B.jpeg", path=here::here("output"),
  )  

  ggsave(
    plot= 	 fp + fp2 ,
    filename="week_overall_combined.jpeg", path=here::here("output"),
  ) 
