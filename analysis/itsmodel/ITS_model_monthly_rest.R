...
its_function <- function(outcomes_vec = outcomes,
  ...
  plot1 <- ggplot(main_plot_data, aes(x = monPlot, y = pc_broad, group = outcome_name)) +
    ...
    geom_vline(xintercept = c(as.Date(abline_min), 
                              as.Date(abline_max)), col = 1, lwd = 1 ,linetype="dotdash") + # 2020-04-05 is first week/data After lockdown gap
    labs(x = "", y = "", title = "A") +
    theme_classic() +
  ...
  plot1
  ggsave(
    plot= plot1, width = 12, height = 12, dpi = 640,
    filename="mon_overall_predicted_rest.jpeg", path=here::here("output"),
  )    
}
