...
main <- function(condition) {
  ...
  for (i in 1:3) {
    ...
    if(i==1){
      mod=clogit(case ~ ab_history_count + strata(set_id), df)
    }
    else if(i==2){
      mod=clogit(case ~ covid*ab_history_count + ab_history_count + covid + strata(set_id), df)
    }
    else if(i==3){
      mod=clogit(case ~ ab_history_count + charlsonGrp + strata(set_id), df)
    }
