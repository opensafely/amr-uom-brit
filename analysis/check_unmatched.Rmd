...
```{r}
df <- read_rds(here::here("output","matched_patients.rds"))
...
df2 <- read_rds(here::here("output","unmatched_patients.rds"))
...
df1$age=ifelse(df1$age>=top5,NA,df1$age)

top95=quantile(df1$age, 0.975,na.rm=T)
df1$age=ifelse(df1$age>=top95,NA,df1$age)

df0=df%>%filter(matched==0)
top5=quantile(df0$age, 0.025,na.rm=T)
df0$age=ifelse(df0$age>=top5,NA,df0$age)

top95=quantile(df0$age, 0.975,na.rm=T)
df0$age=ifelse(df0$age>=top95,NA,df0$age)
