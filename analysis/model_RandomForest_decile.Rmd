...
train$index=as.factor(seq(nrow(train)))
...
valid$index=as.factor(seq(nrow(valid)))
...
#rain_X=train_X%>%select(index)

# combined
#train_X=train_X[,c("patient_id","patient_index_date")]
development <- cbind(train_X$index,decile,prob)
...
# combined
validation<- cbind(valid_X$index,decile,prob)
...
df0=merge(train,development, by="index", all.x=TRUE)
df1=merge(valid,validation, by="index", all.x=TRUE)
nrow(df0)
nrow(df1)
```
