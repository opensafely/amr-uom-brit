...
df0=merge(train,development, by=c("patient_index_date","patient_id"), all.x=TRUE)
df1=merge(valid,validation, by=c("patient_index_date","patient_id"), all.x=TRUE)
nrow(df0)
nrow(df1)
```
...
add confounder
```{r}
df0=merge(df0,DF0, by=c("patient_index_date","patient_id"), all.x=TRUE)
df1=merge(df1,DF1, by=c("patient_index_date","patient_id"), all.x=TRUE)
