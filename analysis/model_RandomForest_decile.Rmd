...
nrow(devel)
devel=devel%>% distinct(patient_id,patient_index_date, .keep_all = T)
nrow(devel)
devel=merge(DF,devel, by=c("patient_id","patient_index_date"), all.x=T)
...
nrow(valid)
valid=valid%>% distinct(patient_id,patient_index_date, .keep_all = T)
nrow(valid)
valid=merge(DF,valid, by=c("patient_id","patient_index_date"), all.x=T)

nrow(valid)
saveRDS(valid,here::here("output","validation.rds"))
