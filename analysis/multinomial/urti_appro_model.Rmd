...
```{r}
age5_spline <- rcs(training$age,knots = 5)
```

```{r}
attributes(age5_spline)$parms
```

```{r}
### use training dataset to develop the model
m1 <- vgam(outcome ~ age5_spline + sex + ethnicity_6 + region + charlsonGrp + imd + ab12b4, family = multinomial(parallel = FALSE, refLevel = 1), data =training)
