...
DF$smoking_cat_3 <- as.factor(DF$smoking_cat_3)                                                  
...
DF$bmi_cat <- relevel(DF$bmi_cat, ref = "healthy weight")
DF$smoking_cat_3 <- relevel(DF$smoking_cat_3, ref = "never")
DF <- DF %>% dplyr::select(outcome,age,sex,ethnicity_6,region,charlsonGrp,imd,ab12b4,bmi_cat,smoking_cat_3, hospital_counts)
...
DF <- DF %>% filter (!is.na(bmi_cat))
DF <- DF %>% filter (!is.na(smoking_cat_3))
DF <- DF %>% ifelse(is.na(hospital_counts),0,hospital_counts)
```
...
m1 <- vgam(outcome ~ rcs(age, Knots.manual) + sex + ethnicity_6 + region + charlsonGrp + imd + ab12b4 + bmi_cat + smoking_cat_3 + hospital_counts,  family = multinomial(parallel = FALSE, refLevel = 1), data =training)
summaryvglm(m1)
```
