...
DF$ab12b4 <- ifelse(is.na(DF$ab12b4),0,DF$ab12b4)
...
DF <- DF %>% dplyr::select(outcome,age,sex,ethnicity_6,region,charlsonGrp,imd,ab12b4,smoking_cat_3, hospital_counts)
...
DF$smoking_cat_3 <- ifelse(is.na(DF$smoking_cat_3),"unknown",DF$smoking_cat_3)
...
m1 <- vgam(outcome ~ rcs(age, Knots.manual) + sex + ethnicity_6 + region + charlsonGrp + imd + ab12b4 + smoking_cat_3 + hospital_counts,  family = multinomial(parallel = FALSE, refLevel = 1), data =training)
summaryvglm(m1)
```
