...
```{r echo=FALSE}
  ...
  obs <- plyr::ddply(gpdata_1,~groups_1,summarise,mean.obs=mean(obs))
  exp <- plyr::ddply(gpdata_1,~groups_1,summarise,mean.pred=mean(pred))
  plot.group.1 <- left_join(obs,exp)
  plot.group.1$outcome.cat <- "p1"
  ...
  obs <- plyr::ddply(gpdata_1,~groups_2,summarise,mean.obs=mean(obs))
  exp <- plyr::ddply(gpdata_1,~groups_2,summarise,mean.pred=mean(pred))
  plot.group.2 <- left_join(obs,exp)
  plot.group.2$outcome.cat <- "p2"
  ...
  gpdata_3 <- cbind(dat.calib.flex.mlr_3,groups_1)
  obs <- plyr::ddply(gpdata_3,~groups_3,summarise,mean.obs=mean(obs))
  exp <- plyr::ddply(gpdata_3,~groups_3,summarise,mean.pred=mean(pred))
  plot.group.3 <- left_join(obs,exp)
  plot.group.1$outcome.cat <- "p3"

  plot.group <- rbind(plot.group.1,plot.group.2,plot.group.3)
  plot.group$anal.meth <- "LRTI"

  plot <- ggplot(plot.group, aes(x = mean.pred, y = mean.obs)) +
    geom_abline(intercept = 0, slope = 1) +
