...
```{r echo=FALSE}
DF <- read_csv(here::here("output","lrti_outcome.csv"))
DF <- DF %>% filter (age>3)
...
p <- predict(m1 , newdata = testing,type="response")
  ...
  dat.calib.flex.mlr$anal.meth <- "LRTI"
...
```{r echo=FALSE}
  ...
  obs <- gpdata_1 %>%
    group_by(groups_1) %>%
    summarise(mean.obs = mean(obs, na.rm = TRUE),
              sd.obs = sd(obs, na.rm = TRUE),
              n.obs = n()) %>%
    mutate(se.obs = sd.obs / sqrt(n.obs),
           lower.ci.obs = mean.obs - qt(1 - (0.05 / 2), n.obs - 1) * se.obs,
           upper.ci.obs = mean.obs + qt(1 - (0.05 / 2), n.obs - 1) * se.obs)
  
  pred <- gpdata_1 %>%
    group_by(groups_1) %>%
    summarise(mean.pred = mean(pred, na.rm = TRUE),
              sd.pred = sd(pred, na.rm = TRUE),
              n.pred = n()) %>%
    mutate(se.pred = sd.pred / sqrt(n.pred),
           lower.ci.pred = mean.pred - qt(1 - (0.05 / 2), n.pred - 1) * se.pred,
           upper.ci.pred = mean.pred + qt(1 - (0.05 / 2), n.pred - 1) * se.pred)
  plot.group.1 <- cbind(obs,pred[,-1])
  
  
  plot <- ggplot(plot.group.1, aes(x = mean.pred, y = mean.obs)) +
    geom_abline(intercept = 0, slope = 1) +
    geom_point(size = 0.5, color = "red") +
    xlab("Predicted") + ylab("Observed") +
    ylim(0, 1)+
    geom_errorbar(aes(ymin=lower.ci.obs, 
                      ymax=upper.ci.obs), width =.005)
  plot 
```
...
```{r echo=FALSE}
  ...
  pred <- gpdata_2 %>%
    ...
    mutate(se.pred = sd.pred / sqrt(n.pred),
           lower.ci.pred = mean.pred - qt(1 - (0.05 / 2), n.pred - 1) * se.pred,
           upper.ci.pred = mean.pred + qt(1 - (0.05 / 2), n.pred - 1) * se.pred)
  plot.group.2 <- cbind(obs,pred[,-1])
  
  
  plot <- ggplot(plot.group.2, aes(x = mean.pred, y = mean.obs)) +
    geom_abline(intercept = 0, slope = 1) +
    geom_point(size = 0.5, color = "red") +
    xlab("Predicted") + ylab("Observed") +
    ylim(0, 1)+
...
```{r echo=FALSE}
  ...
  pred <- gpdata_3 %>%
    ...
    mutate(se.pred = sd.pred / sqrt(n.pred),
           lower.ci.pred = mean.pred - qt(1 - (0.05 / 2), n.pred - 1) * se.pred,
           upper.ci.pred = mean.pred + qt(1 - (0.05 / 2), n.pred - 1) * se.pred)
  plot.group.3 <- cbind(obs,pred[,-1])
  
  
  plot <- ggplot(plot.group.3, aes(x = mean.pred, y = mean.obs)) +
    geom_abline(intercept = 0, slope = 1) +
    geom_point(size = 0.5, color = "red") +
    xlab("Predicted") + ylab("Observed") +
    ylim(0, 1)+
