...
```{r echo=FALSE}
DF <- read_csv(here::here("output","ten_infection_outcome.csv"))
DF <- DF %>% filter (age>3)
...
DF$infection <- relevel(DF$infection, ref = "Sore throat")                                    
...
DF <- DF %>% dplyr::select(outcome,age,sex,infection,ethnicity_6,region,charlsonGrp,imd,ab12b4)
...
p <- predict(m1 , newdata = testing,type="response")
  ...
  dat.calib.flex.mlr$anal.meth <- "Overall"
...
```{r echo=FALSE}
  ...
  quantile(pred_prob_inappro, c(0, .25,.5,.75,1)) 
```
### repeat risk
```{r echo=FALSE}
  pred_prob_repeat <- p[,3]
  quantile(pred_prob_repeat, c(0, .25,.5,.75,1)) 
```
