...
df1[AB]=df1[AB]%>%mutate_all(~replace(., is.na(.), 0)) # recode NA -> 0
#df1$total_ab_yr1=rowSums(df1[AB])
...
AB=colnames(df2[,6:137])
df2[AB]=df2[AB]%>%mutate_all(~replace(., is.na(.), 0)) # recode NA -> 0
df2$total_ab_yr2=rowSums(df2[AB])
...
AB=colnames(df3[,6:137])
df3[AB]=df3[AB]%>%mutate_all(~replace(., is.na(.), 0)) # recode NA -> 0
df3$total_ab_yr3=rowSums(df3[AB])
