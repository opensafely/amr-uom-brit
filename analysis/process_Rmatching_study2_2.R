...
DF1=read_csv("input_outcome_study2_2.csv")

## add variables to extracted cohort:"subclass","case", 
DF2 <- read_rds("matched_patients_study2_2.rds")
...
write_rds(DF_type, here::here("output", "abtype79_study2_2.rds"))
...
write_rds(df[comor], here::here("output", "comor17_study2_2.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_study2_2.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_study2_2.rds"))
...
# extracted dataset after matching
DF1=read_csv("input_outcome_study2_2_2.csv")

## add variables to extracted cohort:"subclass","case", 
DF2 <- read_rds("matched_patients_study2_2.2.rds")
...
DF_type=df%>%select(col,"case","subclass")
write_rds(DF_type, here::here("output", "abtype79_study2_2.2.rds"))
...
comor=c("cancer_comor","cerebrovascular_disease_comor", "chronic_obstructive_pulmonary_comor", "heart_failure_comor", "connective_tissue_comor", "dementia_comor", "diabetes_comor", "diabetes_complications_comor", "hemiplegia_comor", "hiv_comor", "metastatic_cancer_comor", "mild_liver_comor", "mod_severe_liver_comor", "mod_severe_renal_comor", "mi_comor", "peptic_ulcer_comor", "peripheral_vascular_comor")
df$Charlson=rowSums(df[comor])
write_rds(df[comor], here::here("output", "comor17_study2_2.2.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_study2_2.2.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_study2_2.2.rds"))
