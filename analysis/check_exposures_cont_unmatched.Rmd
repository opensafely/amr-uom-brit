...
#BMI
```{r}
ggplot(df, aes(x=bmi, fill=case, color=case,group=case)) +
...
#Charlson
```{r}
ggplot(df, aes(x=Charlson, fill=case, color=case,group=case)) +
  geom_histogram(aes(y=..density..), position="identity")+
  geom_density(alpha=0.5)+
    facet_grid(case~.)
...
#lastABtime
```{r}
ggplot(df, aes(x=lastABtime,fill=case,color=case,group=case)) +
...
#total ab (compare to ab_prescriptions)
```{r}
ggplot(df, aes(x=total_ab,fill=case,color=case,group=case)) +
  geom_histogram(aes(y = ..density..),binwidth = 0.8)+
    geom_density(alpha=0.5)+
  facet_grid(case~.)

df[, as.list(summary(total_ab)), by = case]
```


#ab_prescriptions
```{r}
ggplot(df, aes(x=ab_prescriptions,fill=case,color=case,group=case)) +
  geom_histogram(aes(y = ..density..),binwidth = 0.8)+
    geom_density(alpha=0.5)+
  facet_grid(case~.)
...
```{r}
ggplot(df, aes(x=broad_ab_prescriptions,fill=case,color=case,group=case)) +
  geom_histogram(aes(y = ..density..),binwidth = 0.8)+
    geom_density(alpha=0.5)+
  facet_grid(case~.)
