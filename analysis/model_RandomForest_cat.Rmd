...
rf4<- randomForest(formula = case ~ ., data = train_X, 
                           replace = TRUE,
                           sampsize=c(sample_n*3,sample_n),
                           strata=train_X$case,
                           ntree =  1000,
                           nodesize = 100 ,
                           mtry = 3
...
plot(roc <- roc(train_X$case~ pred.y.2),col=2,add=T)
plot(roc <- roc(train_X$case~ pred.y.3),col=3,add=T)
plot(roc <- roc(train_X$case~ pred.y.4),col=4,add=T)
...
plot(roc <- roc(valid_X$case~ pred.y.2),col=2,add=T)
plot(roc <- roc(valid_X$case~ pred.y.3),col=3,add=T)
plot(roc <- roc(valid_X$case~ pred.y.4),col=4,add=T)
```
