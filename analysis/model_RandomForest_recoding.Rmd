...
train_X <- read_rds(here::here("output","train_X_cat.rds"))
...
```{r }
valid_X <- read_rds(here::here("output","valid_X_cat.rds"))
...
col=c("case","total_ab_group","ab_types_group","exposure_period_group" ,"recent_ab_days_group","broad_ab_prescriptions_group" ,"interval_med_group" , "interval_sd_group")
# abtype
train_X=train_X[col]
valid_X=valid_X[col]
...
dummy var (one-cot encording)
```{r}
dummy <- dummyVars(" ~ .", data=train_X)
newdata <- data.frame(predict(dummy, newdata = train_X)) 
```
...
#  sampsize = c(round(n*0.6), round(n*0.8), round(n*0.9)),
 ...
 # valid_auc = NA)
n=as.numeric(table(train_X$case)[2]*0.8)
