...
df$total_ab_qn_5=as.factor(df$level5) #category
...
# case
df=df%>%filter(br_total_ab_qn_5=="1"|br_total_ab_qn_5=="2"|br_total_ab_qn_5=="3"|br_total_ab_qn_5=="4"|br_total_ab_qn_5=="5"|br_total_ab_qn_5=="without broad ab")

df.plot1=df%>% filter(case=="1")%>%group_by(br_total_ab_qn_5)%>%summarise(percent=n())
names(df.plot1)[1]<- "exp.level"
df.plot1$group="Case"



df.plot2=df%>% filter(case=="0")%>%group_by(br_total_ab_qn_5)%>%summarise(percent=n())
names( df.plot2)[1]<- "exp.level"
