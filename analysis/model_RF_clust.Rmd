...
col=c("total_ab","ab_types","exposure_period" ,"recent_ab_days", "broad_prop" ,"interval_med" , "interval_CV","length_CV","lenght_mean",abtype)
#result = expand.grid(
 #  ntree=c(1000,2000),
...
#for (i in 1:nrow(result)) {
  ...
  rf.model <- randomForest(y=NULL,x=train_X[col,], 
                           replace = TRUE,
                           ntree =  1000,
...
train_X$rf.clusters <- rf.cluster
table(rf.cluster, train_X$case)
```

```{r}

model=clogit(case ~ rf.clusters + CCI + covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass),df) 
