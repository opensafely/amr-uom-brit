...
DF[1,1]=paste0("hospital admission")
DF[1,2]=paste0(round(nrow(df)/5)*5)
```
...
DF[2,1]=paste0("hospital admission (remove NA)")
DF[2,2]=paste0(round(nrow(df)/5)*5)
```
...
DF[3,1]=paste0("cohort: covid hospital admission (incident covid infeciton, so exclude recors outside 1 month)")
DF[3,2]=paste0(round(nrow(df)/5)*5)
```
...
DF[4,1]=paste0("CASE - covid icu or covid death within 1 month")
DF[4,2]=paste0(round(nrow(df1)/5)*5)
```
...
DF[4,1]=paste0("Control - covid hospital without any covid severe outcome within 1 month")
DF[4,2]=paste0(round(nrow(df01)/5)*5)

write.csv(DF, here::here("output","check_process_1.csv"))
