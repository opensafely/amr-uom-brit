...
load_dataset <- function(file_path, condition) {
  col_spec <- cols_only(
  ...
  df <- read_csv(here(file_path, paste0("matched_cases_", condition, ".csv")), col_types = col_spec)
  return(df)
}


# Function to load and prepare the second and third dataset
load_prepared_df <- function(file_path, selected_vars, condition) {
  df <- readRDS(here(file_path, paste0("processed/input_case_", condition, "_abvar.rds"))) %>% select(one_of(selected_vars))
  return(df)
}
...
main <- function(condition) {
  df_cases <- load_dataset("output", condition)
  df_matches <- load_dataset("output", condition)
  ...
  df_cases_ab <- load_prepared_df("output", df_ab_vars, condition)
  df_cases_oth <- load_prepared_df("output", df_oth_vars, condition)

  df_matches_ab <- load_prepared_df("output", df_ab_vars, condition)
  df_matches_oth <- load_prepared_df("output", df_oth_vars, condition)
