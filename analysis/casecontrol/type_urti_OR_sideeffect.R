...
main <- function(condition) {
  ...
  meds <- c("Rx_Amoxicillin", "Rx_Doxycycline", "Rx_Clarithromycin", "Rx_Phenoxymethylpenicillin")
  ...
  df <- df %>% mutate(
    medication = case_when(
      ...
      Rx_Doxycycline == TRUE & total_true == 1 ~ "Doxycycline",
      Rx_Clarithromycin == TRUE & total_true == 1 ~ "Clarithromycin",
      Rx_Phenoxymethylpenicillin == TRUE & total_true == 1 ~ "Phenoxymethylpenicillin",
  ...
  write_csv(medication_freq_pivot, here::here("output", paste0(condition,"_ab_medication_freq_sideeffect.csv")))
  ...
  write_csv(combined_df, here::here("output", paste0(condition,"_ab_type_OR_sideeffect.csv")))
}
