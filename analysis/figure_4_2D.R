...
col_spec1 <-cols_only( Infection_coded_2D_window = col_number(),
...
col_spec2 <-cols_only( Infection_coded_2D_window = col_number(),
                       Broad_given_2D_window = col_number(),
                       date = col_date(format = "")
)
...
df1.20 <- read_csv("measure_infection_coded_2D_window_ab.csv",
               col_types = col_spec1) %>% filter(date >=as.Date("2020-03-01"))
df1.21 <- read_csv("measure_21_infection_coded_2D_window_ab.csv",
               col_types = col_spec1)
df1 <- bind_rows(df1.20,df1.21)

df2.20 <- read_csv("measure_infection_coded_2D_window_broad.csv",
               col_types = col_spec2) %>% filter(date >=as.Date("2020-03-01"))
df2.21 <- read_csv("measure_21_infection_coded_2D_window_broad.csv",
               col_types = col_spec2)
df2 <- bind_rows(df2.20,df2.21)


df <- left_join(df1,df2,by = c("Infection_coded_2D_window","date"))
...
df_coded <- df %>% filter(Infection_coded_2D_window==1)
...
ggsave(p1, width = 12, height = 6, dpi = 640,
                   filename="figure_4_2D_coded.jpeg", path=here::here("output"),
            )  

df_uncoded <- df %>% filter(Infection_coded_2D_window==0)
...
ggsave(p2, width = 12, height = 6, dpi = 640,
                   filename="figure_4_2D_uncoded.jpeg", path=here::here("output"),
            )  



write_csv(df, here::here("output", "figure_4_2D_table.csv"))
