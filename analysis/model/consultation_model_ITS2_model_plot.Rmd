...
df1_counter$time.since=as.factor(0)
...
df2_counter$time.since=as.factor(0)
...
df3_counter$time.since=as.factor(0)
...
df4_counter$time.since=as.factor(0)df4_counter  <- cbind(df4_counter, "resp" = predict(m4, type = "response", se.fit = TRUE, newdata = df1_counter)[1:2])

df5_counter <- subset(df5, select=-c(fit,se.fit))
df5_counter$covid=as.factor(0)
df5_counter$time.since=as.factor(0)
...
df6_counter$time.since=as.factor(0)
...
df7_counter$time.since=as.factor(0)
