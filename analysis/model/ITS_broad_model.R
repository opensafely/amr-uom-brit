...
outcomes <- stringr::str_remove_all(all_files, c("df.model_|.csv"))
...
its_function <- function(outcomes_vec = outcomes,
  ...
  plot_its <- function(outcome){
    ...
    plot1 <- ggplot(main_plot_data, aes(x = weekPlot, y = pc_broad, group = outcome_name)) +
      ...
      labs(y = "% of people prescribed with broad spectrum antibiotics", title = "A") +
      theme_classic() +
    ...
    write.csv(forest_plot_df, file = here::here("data", paste0("its_main_ORs.csv")))
