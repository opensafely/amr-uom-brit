...
all_files <- list.files(here::here("output"), pattern = "df2.model_added_")
outcomes <- stringr::str_remove_all(all_files, c("df2.model_added_|.csv"))
...
for(ii in 1:length(outcomes)){
  load_file <- read.csv(here::here("output", paste0("df2.model_added_", outcomes[ii], ".csv")))
...
its_function <- function(outcomes_vec = outcomes,
  ...
  write_csv(main_plot_data, here::here("output", "its_main_plot_data_uncoded_without_A.csv"))
  ...
  ggsave(
    plot= plot1,
    filename="predicted_plot_uncoded_without_A.jpeg", path=here::here("output"),
  )  
  ...
  write_csv(forest_plot_df, here::here("output", "its_main_ORs_uncoded_without_A.csv"))
  ...
  ggsave(
    plot= fp,
    filename="forest_plot_broad_uncoded_without_A.jpeg", path=here::here("output"),
  )  
