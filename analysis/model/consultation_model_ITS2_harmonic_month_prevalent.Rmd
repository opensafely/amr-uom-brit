---
title: "consultation rate - ITS model_2"
output: 
  html_document:
   toc: true
   toc_depth: 2
...
harmonic <- function(x,per) {
  sin1=sin(2*pi*x/per)
  cos1=cos(2*pi*x/per)
  sin2=sin(4*pi*x/per)
  cos2=cos(4*pi*x/per)
  var=matrix(c(sin1,cos1,sin2,cos2),ncol=4)
  return(var)
}
```
...
## UTI
```{r}
summary(m1.1 <- glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since  , data = df1))
```

## URTI
```{r}
summary(m2.1 <- glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since  , data = df2))
```

## LRTI
```{r}
summary(m3.1 <- glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since , data = df3))
```

## sinusitis
```{r}
summary(m4.1 <- glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since  , data = df4))
```

## otitis externa
```{r}
summary(m5.1 <-glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since , data = df5))
```

## otitis media
```{r }
summary(m6.1 <- glm.nb(counts~ offset(log(population))+ covid + harmonic(month,12) + times + time.since  , data = df6))
```
