...
explanatory<- c("prob")
dependent <- "decile"
case=df%>% filter(case==1)%>% summary_factorlist(dependent, explanatory)
case$case=1
contr=df%>% filter(case==0)%>% summary_factorlist(dependent, explanatory)
contr$case=0
tbl=rbind(case,contr)
...
write.csv(tbl,here::here("output","decile_prop.csv"))
