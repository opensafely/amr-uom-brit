...
df$recent_ab_days= relevel(as.factor(df$df$recent_ab_days), ref="1")
...
model=clogit(case ~ total_ab_group + df$recent_ab_days + total_ab_group*recent_ab_days+ strata(subclass),df) 

summary(model)

set.seed(123)
model=clogit(case ~ total_ab_group + recent_ab_days + total_ab_group*recent_ab_days + CCI + covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass),df) 
