...
all_files <- list.files(here::here("output"), pattern = "df.m.noA.model_")
outcomes <- stringr::str_remove_all(all_files, c("df.m.noA.model_|.csv"))
...
for(ii in 1:length(outcomes)){
  load_file <- read.csv(here::here("output", paste0("df.m.noA.model_", outcomes[ii], ".csv")))
...
its_function <- function(outcomes_vec = outcomes,
  ...
  write_csv(main_plot_data, here::here("output", "its_main_plot_data_monthly_without_A.csv"))
  ...
  ggsave(
    plot= plot1,
    filename="predicted_plot_monthly_without_A.jpeg", path=here::here("output"),
  )  
  ...
  write_csv(forest_plot_df, here::here("output", "its_main_ORs_monthly_without_A.csv"))
  ...
  ggsave(
    plot= fp,
    filename="forest_plot_broad_monthly_without_A.jpeg", path=here::here("output"),
  )  
