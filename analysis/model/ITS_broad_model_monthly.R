...
all_files <- list.files(here::here("output"), pattern = "df.m.model_")
outcomes <- stringr::str_remove_all(all_files, c("df.m.model_|.csv"))
...
its_function <- function(outcomes_vec = outcomes,
  ...
  ggsave(
    plot= fp,
    filename="forest_plot_broad_monthly.jpeg", path=here::here("output"),
  )  
