...
col_spec2 <-cols_only(antibiotic_type = col_character(),
                     AB_given_7D_window = col_number(),
                     Positive_test_event = col_number(),
                     date = col_date(format = "")
)

df20.2 <- read_csv("measure_covid_window_7D_top5_ab_by_type.csv",
                 col_types = col_spec2) %>% filter(date >=as.Date("2020-03-01"))
df21.2 <- read_csv("measure_21_covid_window_7D_top5_ab_by_type.csv",
                 col_types = col_spec2)

df2 <- bind_rows(df20.2,df21.2)

df.frequency2 <- df2 %>% group_by(antibiotic_type) %>% summarise(
  n_count = sum(AB_given_7D_window)
)

all_count <- sum(df2$AB_given_7D_window)
...
write_csv(df.frequency2, here::here("output", "ab_type_table_7D.csv"))
