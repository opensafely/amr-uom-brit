...
df$broad_rate <- round(df$Broad_given_14D_window/df$Positive_test_event,digits = 3)

df$narrow_rate <- round((df$AB_given_14D_window - df$Broad_given_14D_window)/df$Positive_test_event, digits = 3)

df_broad <- select(df,c("broad_rate","date"))
df_broad$group <- "broad"
names(df_broad) <- c("prop","date","group")
df_narrow <- select(df,c("narrow_rate","date"))
df_narrow$group <- "narrow"
names(df_narrow) <- c("prop","date","group")

df_plot <- bind_rows(df_broad,df_narrow)

p <- ggplot(df_plot, aes(x = date, y = prop, fill = group)) + 
  ...
  scale_fill_manual(values = c("#DADAEB", "#9E9AC8"),
                         ...
                         labels=c("broad-spectrum", "narrow-spectrum"))
p

ggsave(p, width = 12, height = 6, dpi = 640,
                   filename="figure_3_prop.jpeg", path=here::here("output"),
            )  

write_csv(df, here::here("output", "figure_3_prop_table.csv"))
