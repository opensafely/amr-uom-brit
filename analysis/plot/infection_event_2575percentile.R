...
# impoprt data
df <- read_csv(
  here::here("output", "measures", "measure_LRTI_event.csv"),
  col_types = cols_only(
...
# calculate= (lrti events)*1000/ population  --per GP per month--
df_gprate <- dfls %>% group_by(practice, cal_mon, cal_year) %>%
  mutate(ab_rate_1000 = value*1000) 
...
plot_percentile <- ggplot(df_mean, aes(x=date))+
  ...
  scale_x_date(date_labels = "%m-%Y", date_breaks = "1 month")+
  theme(axis.text.x=element_text(angle=60,hjust=1))+
  labs(x=NULL, y="LRTI Events Rate per 1000 registered patients")+
  geom_vline(xintercept = as.numeric(as.Date("2019-12-31")), linetype=4)+
  geom_vline(xintercept = as.numeric(as.Date("2020-12-31")), linetype=4)
...
ggsave(
  plot= plot_percentile,
  filename="lrti_event_25th_75th_percentile.png", path=here::here("output"),
)
