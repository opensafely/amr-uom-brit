...
for (i in seq_along(Files)){
  ...
  # filter incident
  dat=dat%>%filter(prevalent==0)%>%select(patient_id,date,infection)# covid infection
...
ggsave(
  plot= abtype_bar,
  filename="ab_recorded_covid_incident_bar.jpeg", path=here::here("output"),
)
ggsave(
  plot= lineplot,
  filename="ab_recorded_covid_incident_line.jpeg", path=here::here("output"),
) 

write_csv(dat, here::here("output", "ab_recorded_covid_incident.csv"))
