...
df <- read_csv(
  here::here("output", "measures", "measure_gp_same_day_pos_ab.csv"),  
  col_types = cols_only(
  ...
  df <- df%>% filter(gp_covid==1)
  
  ## Count the overal Covid Positive Cohort
  df_pos <- df%>% group_by(date,practice,age_cat)%>%
    mutate(total_covid_population=population[gp_covid_ab_prescribed==1]+population[gp_covid_ab_prescribed==0])
  
  df_postive_rate <- df_pos%>%filter(gp_covid_ab_prescribed==1)
...
df_plot_2$rate<- df_plot_2$ab_population/df_plot_2$covid_population
  ...
  ggsave(
  plot= plot,
  filename="same_day_ab_prop_line_gp_age.jpeg", path=here::here("output"),
)  
...
## table 
write_csv(df_plot, here::here("output", "same_day_ab_prop_gp.csv"))