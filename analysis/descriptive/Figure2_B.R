...
df.all <-  df %>% group_by(time,incident_prevalent) %>% summarise(
  numEligible = n(),
)
df.model <- merge(df.repeat_total,df.all,by=c("time","incident_prevalent"))
...
bkg_colour <- "white"
figure_incident_strata <- ggplot(df.model, aes(x = as.Date("2019-01-01"), y = value, group = factor(incident_prevalent), col = factor(incident_prevalent), fill = factor(incident_prevalent))) +
  ...
  labs(x = "Date", y = "% of repeat prescription", title = "", colour = "Infection", fill = "Infection") +
  theme_classic()  +
...
ggsave(
  plot= figure_incident_strata, width = 12, height = 6, dpi = 640,
  filename="Figure2_B.jpeg", path=here::here("output"),
)  

df.model$value <- df.model$numOutcome/df.model$numEligible

write_csv(df.model, here::here("output", "Figure2_B_table.csv"))
