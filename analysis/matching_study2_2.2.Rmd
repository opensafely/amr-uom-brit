...
df1=read_csv(here::here("output","/case_covid_death_study2_2.2.csv"))
df1$case=1
df2=read_csv(here::here("output","control_covid_infection_study2_2.2.csv"))
df2$case=0
...
m.data1=m.data1%>%select("case","subclass","patient_id","patient_index_date","age","sex","stp","cal_YM") # key variables for merging
write_rds(m.data1, here::here("output", "matched_patients_study2_2.2.rds"))

m.data2 <- match.data(m.out1) # unique patient ID
m.data2=m.data2%>%select("patient_id","patient_index_date") 
write.csv(m.data2,here::here("output","matched_patients_id_study2_2.2.csv"))

unmatched=subset(df, !df$patient_id %in% m.data1$patient_id)
write_rds(unmatched,here::here("output","unmatched_patients_study2_2.2.rds"))
