...
col_spec1 <-cols_only(AB_given_7D_window = col_number(),
                       Positive_test_event = col_number(),
                       value = col_number(),
                       date = col_date(format = "")
)

col_spec2 <-cols_only(Broad_given_7D_window = col_number(),
                       date = col_date(format = "")
)
...
df1.20 <- read_csv("measure_covid_window_7D_window_ab.csv",
               col_types = col_spec1) %>% filter(date >=as.Date("2020-03-01"))
df1.21 <- read_csv("measure_21_covid_window_7D_window_ab.csv",
               col_types = col_spec1)
df1 <- bind_rows(df1.20,df1.21)

df2.20 <- read_csv("measure_covid_window_7D_window_broad.csv",
               col_types = col_spec2) %>% filter(date >=as.Date("2020-03-01"))
df2.21 <- read_csv("measure_21_covid_window_7D_window_broad.csv",
               col_types = col_spec2)
...
df$Broad_given_7D_window <- plyr::round_any(df$Broad_given_7D_window, 5)
df$AB_given_7D_window <- plyr::round_any(df$AB_given_7D_window, 5)
df$broad_rate <- round(df$Broad_given_7D_window/df$AB_given_7D_window,digits = 3)
...
ggsave(p, width = 8, height = 4, dpi = 640,
                   filename="figure_3_7D.jpeg", path=here::here("output"),
            )  

write_csv(df, here::here("output", "figure_3_7D_table.csv"))