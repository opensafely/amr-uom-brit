...
## ab_6w label
```{r}
mod1=clogit(case ~ level+ ab_6w + strata(subclass), df)
summary(mod1)
vif(mod1)
```
...
## ab_6w amount
```{r}
mod1=clogit(case ~ level+ total_ab_6w + strata(subclass), df)
summary(mod1)
vif(mod1)
```
...
## ab_6w types
```{r}
mod1=clogit(case ~ level+ ab_types_6w+ strata(subclass), df)
summary(mod1)
vif(mod1)
```
...
## latest ab time
```{r}
mod1=clogit(case ~ level+ lastABtime_6w + strata(subclass), df)
summary(mod1)
vif(mod1)
```
...
mod=clogit(case ~ ab_qn_type + lastABtime_6w + CCI + covrx_ever + bmi_cat + care_home + flu_vaccine + smoking_cat_3 + imd+ ethnicity_6 + strata(subclass), df)
