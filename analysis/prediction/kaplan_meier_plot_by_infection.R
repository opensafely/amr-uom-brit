...
for (col_name in infection_columns) {
  # Filter data based on the current infection column
  infection_data <- data %>% filter(get(col_name) == TRUE)
  ...
  # Generate the plot
  plot_title <- tools::toTitleCase(sub("^has_", "", col_name))  # Convert 'has_uti' to 'Uti', etc.
  Figure1 <- ggplot(surv_df, aes(x = time, y = survival)) +
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = "blue") + 
    geom_line(aes(y = survival), color = "blue", size = 0.5) +
    labs(title = paste("Kaplan-Meier Survival Curve with Confidence Interval for", plot_title),
         x = "Time (days)",
         y = "Survival") +
    theme_light() +
  ...
  # Save the plot
  filename <- paste0("KM_by_", plot_title, "_with_CI.jpeg")
