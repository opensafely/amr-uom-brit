...
df2 <- read_csv("measure_ae_admission.csv",
                col_types = col_spec2)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)
## select and filter infection patient
df3 <- read_csv("measure_uti_ae_admission.csv",
                col_types = col_spec3)%>%filter(uti_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)
# Rename 'ae_admitted' column for each dataset
df3 <- read_csv("measure_uti_ae_admission.csv", col_types = col_spec3) %>%filter(uti_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(uti_ae = ae_admitted)

df4 <- read_csv("measure_lrti_ae_admission.csv", col_types = col_spec4) %>%filter(lrti_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(lrti_ae = ae_admitted)

df5 <- read_csv("measure_urti_ae_admission.csv", col_types = col_spec5) %>%filter(urti_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(urti_ae = ae_admitted)

df6 <- read_csv("measure_sinusitis_ae_admission.csv", col_types = col_spec6) %>%filter(sinusitis_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(sinusitis_ae = ae_admitted)

df7 <- read_csv("measure_ot_externa_ae_admission.csv", col_types = col_spec7) %>%filter(ot_externa_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(ot_externa_ae = ae_admitted)

df8 <- read_csv("measure_ot_media_ae_admission.csv", col_types = col_spec8) %>%filter(ot_media_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
  rename(ot_media_ae = ae_admitted)

df9 <- read_csv("measure_pneumonia_ae_admission.csv", col_types = col_spec9) %>%filter(pneumonia_record==1)%>%group_by(date)%>%summarise(ae_admitted=sum(ae_admitted))%>%select(ae_admitted,date)%>%
...
write_csv(df, here::here("output", "infection_table.csv"))
...
write_csv(df, here::here("output", "infection_forplot.csv"))
...
ggsave(p, width = 10, height = 6, dpi = 640,bg="white",
       filename="figure_infection.jpeg", path=here::here("output"),
)  