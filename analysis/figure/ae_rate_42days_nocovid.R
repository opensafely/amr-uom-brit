...
df2 <- read_csv("measure_2ae_ae_admission.csv",
                col_types = col_spec2)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)
## select and filter infection patient
df3 <- read_csv("measure_2ae_uti_ae_admission.csv", col_types = col_spec3) %>%filter(uti_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(uti_ae = ae_admitted)

df4 <- read_csv("measure_2ae_lrti_ae_admission.csv", col_types = col_spec4) %>%filter(lrti_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(lrti_ae = ae_admitted)

df5 <- read_csv("measure_2ae_urti_ae_admission.csv", col_types = col_spec5) %>%filter(urti_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(urti_ae = ae_admitted)

df6 <- read_csv("measure_2ae_sinusitis_ae_admission.csv", col_types = col_spec6) %>%filter(sinusitis_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(sinusitis_ae = ae_admitted)

df7 <- read_csv("measure_2ae_ot_externa_ae_admission.csv", col_types = col_spec7) %>%filter(ot_externa_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(ot_externa_ae = ae_admitted)

df8 <- read_csv("measure_2ae_ot_media_ae_admission.csv", col_types = col_spec8) %>%filter(ot_media_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
  rename(ot_media_ae = ae_admitted)

df9 <- read_csv("measure_2ae_pneumonia_ae_admission.csv", col_types = col_spec9) %>%filter(pneumonia_record==1)%>%filter(antibiotic_treatment==1)%>%filter(covid_6weeks==0)%>%select(ae_admitted,date)%>%
...
write_csv(df, here::here("output", "ae_rate_42days_nocovid_forplot.csv"))
