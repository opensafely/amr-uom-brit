...
# import data
rm(list=ls())
#df=read_rds("/Users/yayang/Documents/GitHub/amr-uom-brit/output/unmatched_outcome.rds")
...
# br_total_ab_qn_5
```{r}
ggplot(df, aes(x=br_total_ab_qn_5,fill=case,color=case, group=case)) +
  geom_bar(aes(y = ..prop..))+
  facet_grid(case~.)

table(df$case,df$br_total_ab_qn_5)
prop.table(table(df$case,df$br_total_ab_qn_5),margin=1)*100
```
...
# freq table of overall 79 type ab & 17 kind comor
```{r}
df1 <- read_rds(here::here("output","abtype79.rds"))
colSums(df1)

```

```{r}
df2 <- read_rds(here::here("output","comor17.rds"))
...
# compare 79 kind ab and all ab
```{r}
df1$total=rowSums(df1)
df <- read_rds(here::here("output","matched_outcome.rds"))

df=df%>%select(ab_prescriptions)
DF=cbind(df1,df)
DF$diff=DF$ab_prescriptions-DF$total
DF=DF%>%filter(ab_prescriptions==0 & total!=0)
DF=DF[,-c(80:82)]
