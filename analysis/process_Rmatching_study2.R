...
DF1=read_csv("input_outcome_study2.csv")

## add variables to extracted cohort:"subclass","case", 
DF2 <- read_rds("matched_patients_study2.rds")
...
write_rds(DF_type, here::here("output", "abtype79_study2.rds"))
...
write_rds(df[comor], here::here("output", "comor17_study2.rds"))
...
write_rds(df2, here::here("output", "matched_outcome_study2.rds"))


# check again
df=df%>%filter(is.na(ons_died_date_before))
df=df%>%filter(is.na(dereg_date))
write_rds(df, here::here("output", "matched_outcome_check_study2.rds"))
